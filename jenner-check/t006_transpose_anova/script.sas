...
/* --- author's transpose of the per-year ratios into a long dataset, then --- */
/* --- ANOVA (PROC GLM) of GLH-per-learner across the three years --- */
proc transpose data=work.modified
               out=work.transposed(keep = glh_year glh_per_year1)
               name = glh_year
               prefix = glh_per_year;
...
proc means data=work.transposed n mean std fw=5 maxdec=1 nonobs;
    class glh_year;
    var glh_per_year1;
run;

title 'Applying ANOVA On Years and glh/learner';
proc glm data=work.transposed;
    class glh_year;
