...
/* --- author's verification prints of the imported + merged data --- */
title 'Sixth Form college Dataset';
proc print data=work.sixthform label;
run;

title 'Further Education dataset';
proc print data=work.fedata label;
run;

title 'Dataset After Merging';
proc print data=work.merged label;
run;

title 'Looking Into Missing Value Count';
proc means data=work.merged n nmiss min max maxdec=0;
    var Glh_year_1 learners_year_1 Glh_year_2 learners_year_2 Glh_year_3 learners_year_3;
run;
